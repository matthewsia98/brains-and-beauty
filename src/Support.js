...
const Support = () => {
    ...
    const contact_form = () => {
        ...
        reasons.forEach(reason => {
            ...
            reason_options.push(<option value={translated_reason}>{translated_reason}</option>)
        })
        ...
        return (
            <form id="contact-form" className="mx-4">
                <div className="row">
                    <div className="col-6">
                        <input type="text" id="first-name" className="form-control" placeholder={language === "English" ? "First name" : "Prénom"} required/>
                    </div>
                    <div className="col-6">
                        <input type="text" id="last-name" className="form-control" placeholder={language === "English" ? "Last name" : "Nom de famille"} required/>
                    </div>
                </div>
                <div className="row mt-4">
                    <div className="col">
                        <input type="text" id="email" className="form-control" placeholder={language === "English" ? "Email" : "E-mail"} required/>
                    </div>
                </div>
                <div className="row mt-4">
                    <div className="form-group col">
                        <label for="reason">{language === "English" ? "Reason for contacting us" : "Raison de nous contacter"}</label>
                ...
                <div className="row mt-4">
                    <div className="form-group col">
                        ...
                        <textarea id="problem-description" className="form-control mt-2" rows="8" placeholder=""/>
                    </div>
                </div>
                <div className="col-auto mt-4 text-center">
                    <button type="submit" className="submit-button" onClick={submit_form}>{language === "English" ? "Submit" : "Soumettre"}</button>
                </div>
            </form>
        )
    }
    ...
    return (
        <div>
            <div className="mt-3 me-4 text-end">
                <button className="btn btn-primary" onClick={toggle_language}>{language === "English" ? "Français" : "English"}</button>
            </div>
            <div>
                <h1 className="text-center">{language === "English" ? "Frequently Asked Questions": "Questions fréquemment posées"}</h1>
                {accordion()}
            </div>
            <div id="contact-us">
                <h1 className="mt-5 mb-4 text-center">{language === "English" ? "Contact Us": "Contactez-nous"}</h1>
                {contact_form()}
            </div>
        </div>
    )
